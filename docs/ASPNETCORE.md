...
**Grace windows (v0.10.1, opt-in).** Single-use per action is the right default and the wrong ergonomics twice over: a settings page costs three prompts in a row, and signing in with 2FA and then opening a protected page costs two prompts within seconds. One option answers both: `StepUp.GraceWindow` — how long a *passed 2FA challenge* keeps satisfying protected calls. `TimeSpan.Zero` by default, validated at startup to be ≤ `Ttl`. A step-up confirmation and a 2FA login prove the same thing, so they are timed the same rather than split across two settings. The evaluator returns `Satisfied` without spending the nonce, so once the window closes the token still has its single use left and then it's done; `/challenge/verify` mints a step-up token beside the handoff token, and the client caches it exactly like a step-up one.

**A recovery-code login never gets one.** `/challenge/recovery-code` returns no step-up token whatever `GraceWindow` says: that is the flow of someone who lost their factors, which is also the flow of someone who took the account over, and the first thing they would reach for is removing the owner's methods.

The window travels **inside the token** (an `omni2fa_grace_until` NumericDate claim) rather than in a server-side "this user confirmed recently" record, and that choice is the security argument: a per-user record would hand the free pass to *every* session of that user, including a stolen one — which is the exact attack step-up exists to stop. In the token it can only be reused by the browser that did the confirming, and the subject check still rejects it for anyone else. What the window does cost: an attacker with script execution in that page has a reusable confirmation for the duration instead of a single-use one (they could already drive the action in the live page, so the delta is small). Keep it in minutes — `Ttl` itself is uncapped, so setting both to hours quietly turns the barrier into a formality. It is not a "remember this browser" feature; that is roadmap v1.3.

Frontends need no matching setting: the server returns the window with each confirmation (`graceUntil` / `stepUpGraceUntil`) and the client honours exactly that. It caches in memory only, so the confirmation dies with the tab — a host that hard-reloads after login loses the login-granted one and prompts as before.
