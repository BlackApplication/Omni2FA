...
**A user population is a first-class boundary.** Hosts that authenticate several populations (staff and customers of the same application, separate identity tables, overlapping ids) declare each as an *audience*: a route prefix, a subject namespace, and an auth scheme, always configured together. The library applies the namespace itself — mounts tag their endpoints, the default `IUserContextAccessor` reads that tag, and `IOmni2FaAudienceRegistry` gives the host the same translation where it talks to Omni2FA outside a mount (login, finalize). Rationale: id collision between two populations is a silent authentication failure — one user reading another's factors — and "remember to prefix the subject everywhere" is exactly the kind of host discipline this library exists to remove. The default audience is implicit, so single-population hosts see no change. Details and the trade-offs in [`ASPNETCORE.md`](ASPNETCORE.md) §7.
...
**Grace window (v0.10.1).** `StepUpOptions.GraceWindow` (zero by default) is how long a passed 2FA challenge keeps satisfying protected calls — one setting for both ceremonies, since a step-up confirmation and a 2FA login prove the same thing. It is expressed as a claim inside the token rather than a server-side "this user confirmed recently" record, and the reason is the threat model: a per-user record would grant the pass to every session of that user including a stolen one — precisely what step-up defends against — while a claim in the token can only be reused by the browser holding it, and the subject check still applies. The evaluator skips the nonce consume inside the window, so the token keeps its single use for after it. A recovery-code login is deliberately excluded: it is the flow of an account already in trouble. It is validated at startup to be ≤ `Ttl`; the server returns the window with each confirmation, so frontends hold no matching setting and cannot drift out of sync.
...
- **2026-07-27** — v0.10.0: added the audience boundary to section 7 (mount + subject namespace + auth scheme per user population), after a live host had to flag its portal's 2FA requests with a custom header and hand-prefix subjects in three separate places. Design notes in `ASPNETCORE.md` §7.
